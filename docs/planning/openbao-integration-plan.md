...
- **SecretRef exec-provider wiring** (`secrets.providers.openbao` block rendered into `openclaw.json` pointing at a `bao`-CLI-compatible resolver): requires either bundling the `bao` CLI binary or a small Node resolver script into the **agent image** (`agent/instance/Dockerfile`), which is a separate build pipeline from control-plane wiring. v1 ships `OPENBAO_ADDR`/`OPENBAO_TOKEN` env vars only — sufficient for any script/exec-tool use via plain HTTP (`curl`) or the `bao` CLI if an operator installs it themselves; OpenClaw's own SecretRef-eligible config fields (model API keys etc.) can't yet point at OpenBao until this lands.
